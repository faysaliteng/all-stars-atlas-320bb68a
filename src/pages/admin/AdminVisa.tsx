...
const VisaFormSettingsEditor = () => {
  ...
  const addCountry = () => {
    updateConfig(cfg => {
      cfg.countries.push({
        code: `country_${Date.now()}`,
        name: "New Country",
        flag: "🏳️",
        visaTypes: ["Tourist"],
        processingOptions: [{ label: "Normal", days: "5-7 days", extraFee: 0 }],
        baseFee: 3000,
        serviceFee: 1000,
        requiredDocs: ["Valid passport (min 6 months validity)"],
        active: true,
      });
    });
  };
  ...
  const removeCountry = (idx: number) => {
    updateConfig(cfg => { cfg.countries.splice(idx, 1); });
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input value={current.pageTitle} onChange={e => setEditData(prev => prev ? { ...prev, pageTitle: e.target.value } : prev)} />
            </div>
            <div className="space-y-1.5">
              <Label>Processing Note</Label>
              <Input value={config.estimatedProcessingNote} onChange={e => updateConfig(c => { c.estimatedProcessingNote = e.target.value; })} />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label>Terms Checkbox Text</Label>
            <Textarea value={config.termsText} onChange={e => updateConfig(c => { c.termsText = e.target.value; })} rows={2} />
          </div>
          <div className="space-y-1.5">
            <Label>Step Labels</Label>
            <div className="flex flex-wrap gap-2">
              {config.formSteps.map((s, i) => (
                <Input key={i} value={s.label} className="w-36" onChange={e => updateConfig(c => { c.formSteps[i].label = e.target.value; })} />
              ))}
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold">Countries & Pricing</h2>
        <Button size="sm" onClick={addCountry}><Plus className="w-4 h-4 mr-1" /> Add Country</Button>
      </div>
          ...
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-base">{country.flag} {country.name}</CardTitle>
                <Badge variant="outline" className={country.active ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}>
                  {country.active ? "Active" : "Inactive"}
                </Badge>
              </div>
              <div className="flex items-center gap-2">
                <Switch checked={country.active} onCheckedChange={v => updateCountry(ci, c => { c.active = v; })} />
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => removeCountry(ci)}>
          ...
          <CardContent className="space-y-4">
            <div className="grid sm:grid-cols-4 gap-3">
              <div className="space-y-1.5">
                ...
                <Input value={country.name} onChange={e => updateCountry(ci, c => { c.name = e.target.value; })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Code</Label>
                <Input value={country.code} onChange={e => updateCountry(ci, c => { c.code = e.target.value; })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Flag Emoji</Label>
                <Input value={country.flag} onChange={e => updateCountry(ci, c => { c.flag = e.target.value; })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Base Fee (৳)</Label>
                <Input type="number" value={country.baseFee} onChange={e => updateCountry(ci, c => { c.baseFee = Number(e.target.value); })} />
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" value={country.serviceFee} onChange={e => updateCountry(ci, c => { c.serviceFee = Number(e.target.value); })} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Visa Types (comma-separated)</Label>
                <Input value={country.visaTypes.join(", ")} onChange={e => updateCountry(ci, c => { c.visaTypes = e.target.value.split(",").map(s => s.trim()).filter(Boolean); })} />
              </div>
            </div>

            {/* Processing Options */}
            <div>
              <Label className="text-xs font-semibold">Processing Options</Label>
              <div className="space-y-2 mt-1.5">
                {country.processingOptions.map((po, pi) => (
                  <div key={pi} className="flex items-center gap-2">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive shrink-0" onClick={() => updateCountry(ci, c => { c.processingOptions.splice(pi, 1); })}>
                      <Trash2 className="w-3.5 h-3.5" />
                    </Button>
                  </div>
                ))}
                <Button variant="outline" size="sm" onClick={() => updateCountry(ci, c => { c.processingOptions.push({ label: "Rush", days: "1 day", extraFee: 3000 }); })}>
            ...
            <div>
              ...
              <div className="space-y-2 mt-1.5">
                {country.requiredDocs.map((doc, di) => (
                  <div key={di} className="flex items-center gap-2">
                    <Input value={doc} className="flex-1" onChange={e => updateCountry(ci, c => { c.requiredDocs[di] = e.target.value; })} />
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive shrink-0" onClick={() => updateCountry(ci, c => { c.requiredDocs.splice(di, 1); })}>
                      <Trash2 className="w-3.5 h-3.5" />
                    </Button>
                  </div>
                ))}
                <Button variant="outline" size="sm" onClick={() => updateCountry(ci, c => { c.requiredDocs.push("New document requirement"); })}>
