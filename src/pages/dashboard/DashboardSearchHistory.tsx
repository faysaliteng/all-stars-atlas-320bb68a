...
import { getCollection, removeFromCollection } from "@/lib/local-store";
import { useNavigate } from "react-router-dom";

const STORE_KEY = "dashboard_search_history";
...
const DashboardSearchHistory = () => {
  ...
  const [searches, setSearches] = useState(() => getCollection(STORE_KEY, mockSearchHistory.data));

  const filtered = searches.filter((item: any) => {
    if (filter !== "all" && item.type !== filter) return false;
    if (search) {
      const q = search.toLowerCase();
      return (item.summary || "").toLowerCase().includes(q) || (item.destination || "").toLowerCase().includes(q);
    }
    return true;
  });

  const clearAll = () => {
    localStorage.removeItem(`st_${STORE_KEY}`);
    setSearches([]);
